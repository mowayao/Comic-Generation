...
import pickle
def load():
	raw_data = pd.read_csv('/media/mowayao/data/comic_generation/tags_clean.csv', header = None).as_matrix()
...
def split_number(mylist):
	...
	try:
		t = mylist[max_idx][0].split(" ")
	except IndexError:
		print "IndexError", max_idx
...
def tag_to_id():
	...
	pickle.dump(hair_dict, open("hair_dict", "wb"))
	pickle.dump(eyes_dict, open("eyes_dict", "wb"))
	return data
def hair_color():
	tags = []
	i = 0
